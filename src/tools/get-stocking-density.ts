...
function formatResult(
  ...
  const result: Record<string, unknown> = {
    ...
    _citation: buildCitation(
      `Stocking density: ${densities[0].species_name}`,
      `Stocking density for ${densities[0].species_name}`,
      'get_stocking_density',
      { species: densities[0].species_id },
    ),
  };
...
export function handleGetStockingDensity(db: Database, args: StockingArgs) {
  ...
  // Fallback: if filtered query returned nothing, retry without optional
  // filters and include available values so the LLM can self-correct.
